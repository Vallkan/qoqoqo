...
contract PokemonToken is ERC721 {
  ...
  function purchase(uint256 _tokenId) public payable {
    ...
    if (sellingPrice < firstStepLimit) {
      // first stage
      pokemonIndexToPrice[_tokenId] = SafeMath.div(SafeMath.mul(sellingPrice, 200), 94);
    } else if (sellingPrice < secondStepLimit) {
      // second stage
      pokemonIndexToPrice[_tokenId] = SafeMath.div(SafeMath.mul(sellingPrice, 120), 94);
    } else {
      // third stage
      pokemonIndexToPrice[_tokenId] = SafeMath.div(SafeMath.mul(sellingPrice, 115), 94);
    }
    ...
    TokenSold(_tokenId, sellingPrice, pokemonIndexToPrice[_tokenId], oldOwner, newOwner, pokemons[_tokenId].name);
